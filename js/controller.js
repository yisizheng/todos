(function(angular) {
  ...
  todoApp.controller('MainController', [
    ...
    function($scope, $location, Storage) {
      ...
      // 从$window.localStorage获取任务列表，相当于一张表 考虑几个字段的问题
      $scope.todos = Storage.get(); // x00001

      // ===== 暴露行为 =====

      // 新增任务，最上面输入框按回车新增任务逻辑
      $scope.add = function() {
      	// 此方法会在用户敲回车时触发
      	// 回车之前用户肯定已经输入了值
        ...
        Storage.add($scope.input); // 添加到todos列表 并且存起来，add方法在severce里封装，
        //就是上面的.push,然后save（）方法存到$window.localStorage中
        $scope.input = '';
      };
